package com.mingh.learn.design.principle;
...
public interface DesignPrincipleExample {
     ...
     *
     * 概论:
     *      用抽象构建框架, 用实现扩展细节。SRP 告诉我们实现类的职责要单一; LSP 告诉我们不要破坏继承体系; DIP 告诉我们要面向接口编程; ISP 告诉我们在
     *      接口设计时要精简单一; LoD 告诉我们低耦合。而开闭原则是总纲, 它告诉我们对扩展开放, 对修改关闭。
     *
     * 参考:
     *      https://blog.csdn.net/yucaixiang/article/details/90239817
     *      https://blog.csdn.net/AE86JayChou/article/details/85796229
 ...
 *      3. 单一职责原则是实现高内聚, 低耦合的指导方针, 理解简单但运用起来就不简单了
 *      思考: 为什么说运用起来不简单?
 *          因为有职责扩散。所谓职责扩散, 就是因为某种原因, 职责 P 被分化为粒度更细的 P1 和 P2, 而 P1 或 P2 还可能进行扩散
 * 优点:
 *      1. 降低类的复杂度, 使逻辑更加清晰
 ...
 *  理解:
 *      1. LSP 是继承复用的基石, 只有当衍生类可以替换掉基类, 软件单位的功能不受影响时, 基类才能真正被复用
